...
export default function TypePage({ onBack, onSelectType }) {
  ...
  const theoryRow1 = type4Data.coreModules.slice(0, 4);
  const theoryRow2 = type4Data.coreModules.slice(4, 8);
  ...
  <button
    ...
    Typ {nextType} →
      ...
      <header className="rathmer-home-header">
        ...
        <h1>Typ 4 – Der Individualist</h1>

        <p>
          Eine interaktive Übersicht der Typ-4-Struktur, ihrer Grunddynamik und
          Subtypen.
        </p>
      </header>
          ...
          return (
            <div key={module.label}>
              <button
                ...
                style={{
  borderColor: active
    ? TYPE_SHADOWS[type4Data.type]
    : `${TYPE_SHADOWS[type4Data.type]}55`,
}}
                onClick={() =>
                  toggleItem(
                    openTheoryRow2Items,
      ...
      <section className="rathmer-main-grid">
        {type4Data.subtypes.map((subtype) => (
          <div key={subtype.code} className="rathmer-subtype-column">
            ...
            style={{
  background: TYPE_GRADIENTS[type4Data.type],
  boxShadow: `0 16px 30px ${TYPE_SHADOWS[type4Data.type]}`,
}}
              onClick={() => {
  ...
  style={{
    background: selectedSubtypeItem
      ? undefined
      : TYPE_INFO_GRADIENTS[type4Data.type],
  }}
>
          {selectedSubtypeItem ? (
            <>
