...
const currentType = type6Data.type;

const prevType = currentType === 1 ? 9 : currentType - 1;
const nextType = currentType === 9 ? 1 : currentType + 1;
  
  const theoryRow1 = type6Data.coreModules.slice(0, 4);
  const theoryRow2 = type6Data.coreModules.slice(4, 8);
  ...
  onClick={() => onSelectType(nextType)} >
    Typ {nextType} →
      ...
      <header className="rathmer-home-header">
        ...
        <h1>Typ 6 – Der loyale Skeptiker</h1>

        <p>
          Eine interaktive Übersicht der Typ-6-Struktur, ihrer Grunddynamik und
          Subtypen.
        </p>
      </header>
          ...
          return (
            <div key={module.label}>
              <button
                ...
                style={{
  borderColor: TYPE_SHADOWS[type6Data.type],
}}
                onClick={() =>
                  toggleItem(
                    openTheoryRow2Items,
      ...
      <section className="rathmer-main-grid">
        {type6Data.subtypes.map((subtype) => (
  ...
  style={{
    background: selectedSubtypeItem
      ? undefined
      : TYPE_INFO_GRADIENTS[type6Data.type],
  }}
>
          {selectedSubtypeItem ? (
            <>
