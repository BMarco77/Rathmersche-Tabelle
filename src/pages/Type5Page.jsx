...
export default function TypePage({ onBack, onSelectType }) {
  ...
  const currentType = type5Data.type;

const prevType = currentType === 1 ? 9 : currentType - 1;
const nextType = currentType === 9 ? 1 : currentType + 1;
  
  const theoryRow1 = type5Data.coreModules.slice(0, 4);
  const theoryRow2 = type5Data.coreModules.slice(4, 8);
  ...
  <button
    ...
    Typ {nextType} →
      ...
      <header className="rathmer-home-header">
        ...
        <h1>Typ 5 – Der Denker</h1>

        <p>
          Eine interaktive Übersicht der Typ-5-Struktur, ihrer Grunddynamik und
          Subtypen.
        </p>
      </header>
          ...
          return (
            <div key={module.label}>
              <button
                ...
                style={{
  borderColor: active
    ? TYPE_SHADOWS[type5Data.type]
    : `${TYPE_SHADOWS[type5Data.type]}55`,
}}
                onClick={() =>
                  toggleItem(
                    openTheoryRow2Items,
      ...
      <section className="rathmer-main-grid">
        {type5Data.subtypes.map((subtype) => (
          <div key={subtype.code} className="rathmer-subtype-column">
            <button
             ...
             style={{
  background: TYPE_GRADIENTS[type5Data.type],
  boxShadow: `0 16px 30px ${TYPE_SHADOWS[type5Data.type]}`,
}}
              onClick={() => {
  ...
  style={{
    background: selectedSubtypeItem
      ? undefined
      : TYPE_INFO_GRADIENTS[type5Data.type],
  }}
>
          {selectedSubtypeItem ? (
            <>
