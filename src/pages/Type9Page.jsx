...
export default function TypePage({ onBack, onSelectType }) {
  ...
  const currentType = type9Data.type;

  const prevType = currentType === 1 ? 9 : currentType - 1;
  const nextType = currentType === 9 ? 1 : currentType + 1;
  
  const theoryRow1 = type9Data.coreModules.slice(0, 4);
  const theoryRow2 = type9Data.coreModules.slice(4, 8);
  ...
  <button
    ...
    Typ {nextType} →
      ...
      <header className="rathmer-home-header">
        ...
        <h1>Typ 9 – Der Vermittler</h1>

        <p>
          Eine interaktive Übersicht der Typ-9-Struktur, ihrer Grunddynamik und
          Subtypen.
        </p>
      </header>
          ...
          return (
            <div key={module.label}>
              <button
                ...
                style={{
  borderColor: active
    ? TYPE_SHADOWS[type9Data.type]
    : `${TYPE_SHADOWS[type9Data.type]}55`,
}}
                onClick={() =>
                  toggleItem(
                    openTheoryRow2Items,
      ...
      <section className="rathmer-main-grid">
        {type9Data.subtypes.map((subtype) => (
          <div key={subtype.code} className="rathmer-subtype-column">
            <button
              ...
              style={{
  background: TYPE_GRADIENTS[type9Data.type],
  boxShadow: `0 16px 30px ${TYPE_SHADOWS[type9Data.type]}`,
}}
              onClick={() => {
  ...
  style={{
    background: selectedSubtypeItem
      ? undefined
      : TYPE_INFO_GRADIENTS[type9Data.type],
  }}
>
          {selectedSubtypeItem ? (
            <>
